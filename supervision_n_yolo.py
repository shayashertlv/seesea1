...
WEIGHTS_PATH = os.getenv("YOLO_WEIGHTS_PATH", "/workspace/runs_y11x/mix_ftA_clean/weights/best.pt")
...
# (Old) Ultralytics tracker knobs – now mapped to Supervision.ByteTrack
TRACK_CONF = float(os.getenv("TRACK_CONF", "0.35"))  # -> ByteTrack.track_thresh
TRACK_IOU = float(os.getenv("TRACK_IOU", "0.9"))  # used in detector NMS, not tracker (we pass to YOLO inference)
...
# Global handle for trajectory predictor (set in run_tracking_with_supervision)
traj_predictor = None
